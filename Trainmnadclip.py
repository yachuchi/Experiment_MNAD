...
print(f"number of the memory items:{args.msize}")
print(f"train_folder:{train_folder}")
print(f"lr:{args.lr}")
...
for epoch in range(args.epochs):
    ...
    for j,(imgs) in enumerate(train_batch):
        ...
        mnad_loss = loss_pixel + args.loss_compact * compactness_loss + args.loss_separate * separateness_loss
        similarity_loss = torch.tensor(1-similarity[cnt], requires_grad=True)
        loss = (mnad_loss + similarity_loss)/2
