...
similarity = np.load('./data/similarity_walk_test.npy')
...
for k,(imgs) in enumerate(test_batch):
    ...
    if args.method == 'ae':
        ...
        # mse_imgs = torch.mean(loss_func_mse(outputs, imgs)).item()
        mse = torch.mean(loss_func_mse((outputs[0]+1)/2, (imgs[0]+1)/2)).item()
        # similarity_loss = torch.tensor(similarity[cnt], requires_grad=True)   

    # mse_list[videos_list[video_num].split('/')[-1]].append(psnr(mse)) #psnr
    mse_list[videos_list[video_num].split('/')[-1]].append(mse) #mse
    # mse_list[videos_list[video_num].split('/')[-1]].append((mse+(1-similarity_loss))/2) #mse+similarity
    # cnt+=1
...
for video in sorted(videos_list):
    video_name = video.split('/')[-1]
    anomaly_score_total_list += mse_list[video_name]

anomaly_score_total_list = np.asarray(anomaly_score_total_list)

normalscore = []
for i in range(len(similarity)):
    normalscore.append(similarity[i])

normalscore = np.asarray(normalscore)
for i in range(len(anomaly_score_total_list)):
    anomaly_score_total_list[i] = (anomaly_score_total_list[i] + normalscore[i])/2

# anomaly_score_total_list = anomaly_score_total_list.detach().numpy()
